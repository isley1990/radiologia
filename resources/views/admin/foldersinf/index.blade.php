        ...
        <p>
            ...
            @if(!is_null(Auth::getUser()->role_id) && config('quickadmin.can_see_all_records_role_id') == Auth::getUser()->role_id)
                @if(Session::get('Folder.filter', 'all') == 'my')
                    <a href="?filter=all" class="btn btn-success">Ver Todos</a>
                @else
                    <a href="?filter=my" class="btn btn-default">Filtrar</a>
                @endif
            @endif
        </p>
    @endcan
                ...
                @if (count($folders) > 0)
                    @foreach ($folders as $folder)
                        <tr data-entry-id="{{ $folder->id }}">
                            ...
                            <td field-key='name'>
                             @can('folder_view')
                                    <a href="{{ route('admin.folders.show',[$folder->id]) }}">{{$folder->cedula}}</a></td>
