...
<link rel="stylesheet" href="{{ asset('css/marcos.css') }}" />
    ...
    <div class="panel-heading" style="height:40px;">
        <div class="col-md-6 col-lg-14">
            <div class="subscribe-text mb-20">
                ...
                @endif
                /{{Auth::getUser()->name}}/Crear Nueva Petición
    ...
    <div class="panel panel-default">
                ...
                @if (count($files) > 0)
                    @foreach ($files as $file)
                        <tr data-entry-id="{{ $file->id }}">
                           ...
                           <td><a href="{{url('/admin/' . $file->uuid . '/download')}}" target="_blank"><span class="glyphicon glyphicon-search" aria-hidden="true"> <img src="{{url('/admin/' . $file->uuid. '/download' )}}" alt="" width="200" height="150"></span> </a> </td>
                            @if( request('show_deleted') == 1 )
                                <td>
                                    @if (Auth::getUser()->role_id == 2 && $userFilesCount >= 5)
                                        @can('file_delete')
        ...
        'route' => ['admin.files.perma_del', $file->id])) !!}
                            ...
                            @else
                                <td>
                                    <a href="{{url('/admin/' . $file->uuid . '/download')}}" class="btn btn-xs btn-success">Descargar</a>
                                    @can('file_delete')
                                        {!! Form::open(array(
                                                                                'style' => 'display: inline-block;',
                                                                                'method' => 'DELETE',
